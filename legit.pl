...
#Function to check if a file exists in the CWD
sub cwd_file_exists{
    my ($file) = @_;
    my $curr_dir = getcwd();
    if (defined "$curr_dir/$file"){
        return 1;
    } 
    return 0;
}

#Function to check if a file exists in the commit's folders
sub commits_file_exists{
    my ($file) = @_;
    my $file_in_commits = getLatestCommitedVersion($file);
    if($file_in_commits ne -1){
        #File in commits exists
        return 1;
    }
    return 0;
}

sub getLatestCommitedFolder{
    my @num_arr; 
    foreach my $file (glob ".legit/commit_?"){
        $file =~ /commit_([0-9]+)/;
        my $commit_num = $1;
    ...
    @num_arr = sort {($b) <=> ($a)} @num_arr;
    print "Last commit folder number = $num_arr[0]\n";
    return $num_arr[0];
}          
...
sub legit_rm{
    ...
    if (defined $ARGV[0] and $ARGV[0] eq "--force"){
        if(defined $ARGV[1] and $ARGV[1] eq "--cached"){
            print "Both forced and cached\n";
            #overide all checks and delete from index only
        }else{
            print "Only forced\n";
            #do not print any warnings
            #removes only from index
            shift @ARGV; #removing the --forced
            my $file_index_exists = 0;
            my $file_in_cwd;
            my $file_in_commits;
            my $file_in_index;
            ...
            foreach my $file (@ARGV){
                print "File is =  $file\n";
                my $file_index_exists = 0;
                my $file_cwd_exists = cwd_file_exists($file);
                my $file_commits_exists = commits_file_exists($file);
                $file_in_commits = getLatestCommitedVersion($file);
                $file_in_cwd = "$curr_dir/$file_in_commits";
                print "File in cwd = $file_in_cwd\n";

                foreach my $index_file (glob ".legit/index/*"){
                    if(basename($index_file) eq basename($file)){
                        #File in index exists
                        $file_index_exists = 1;
                        printf "File in index = $index_file\n";
                        $file_in_index = $index_file;
                        last;
                    }
                }
                    

                if(($file_commits_exists == 0) and ($file_index_exists == 0)){
                    print "legit.pl: error: '$file' is not in the legit repository\n";
                }elsif(($file_commits_exists == 1) or ($file_index_exists == 1)){
                    #Create a tombstone file
                    my $curr_dir = getcwd();
                    if($file_commits_exists){
                        my $latestCommitNum = getLatestCommitedFolder();
                        print "$curr_dir/.legit/commit_$latestCommitNum/$file.tombstone\n";
                        open F, '>', "$curr_dir/.legit/commit_$latestCommitNum/$file.tombstone" or die "$!: Can't create tombstone\n";
                    }
                
                    if($file_index_exists){
                        unlink "$curr_dir/.legit/index/$file";
                    }
                    #my $file = "$curr_dir/$file_in_commits.tombstone.txt";
                }
            }
        }
 
    }elsif((defined $ARGV[0]) and ($ARGV[0] eq "--cached")){
        #remove only from index
        shift @ARGV; #removing --cached
        print "Only cached\n";
        ...
        foreach my $file(@ARGV){
            ...
            if(($file_commits_exists == 0) and ($file_index_exists == 0)){
                print "legit.pl: error: '$file' is not in the legit repository\n";
            }elsif(($file_commits_exists == 1) and ($file_index_exists == 1) and ($file_cwd_exists == 1) 
            and (compare($file_in_index, $file_in_commits) != 0 ) and (compare($file_in_cwd, $file_in_commits) != 0 ) and (compare($file_in_cwd, $file_in_index) != 0)){
        ...
        foreach my $file (@ARGV){
            ...
            #Error messages rm
            if($file_index_exists == 0 and $file_commits_exists == 0 ){
                print "legit.pl: error: '$file' is not in legit repository\n";
            }elsif(($file_commits_exists == 1)  and ($file_index_exists == 1) and (compare($file_in_index,$file_in_commits) != 0)
            and (compare($file_in_index, $file_in_cwd) == 0) and (compare($file_in_commits, $file_in_cwd) != 0)){
                print "legit.pl: error: '$file' has changes staged in the index\n" ;
            }elsif(($file_commits_exists == 1) and ($file_index_exists == 1) and ($file_cwd_exists == 1)
            and (compare($file_in_index, $file_in_commits) != 0 ) and (compare($file_in_cwd, $file_in_commits) != 0 )){
                print "legit.pl: error: '$file' in index is different to both working file and repository\n";
            }elsif(($file_commits_exists == 1) and ($file_cwd_exists == 1) and (compare($file_in_cwd,$file_in_commits) != 0)){
                print "legit.pl: error: '$file' in repository is different to working file\n" ;
            }elsif(($file_commits_exists == 0) and ($file_index_exists == 1) and (compare($file_in_index,$file_in_commits) != 0) ){
                print "legit.pl: error: '$file' has changes staged in the index\n" ;
            }else{
                #remove the files
                my $latestCommitNum = getLatestCommitedFolder();
                print "$curr_dir/.legit/commit_$latestCommitNum/$file.tombstone\n";
                open F, '>', "$curr_dir/.legit/commit_$latestCommitNum/$file.tombstone" or die "$!: Can't create tombstone\n";
                unlink $file_in_cwd if ($file_cwd_exists == 1);
                unlink $file_in_index if ($file_index_exists == 1);
            }
        }
    }
}
